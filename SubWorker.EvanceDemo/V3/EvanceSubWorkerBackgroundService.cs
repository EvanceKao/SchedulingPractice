...
namespace SubWorker.EvanceDemo.V3
{
    ...
    {
        private static readonly int _executorCount = 12;
        private Squad<ScheduleJobExecutor> _squad;
        ...
        {
            _squad = new Squad<ScheduleJobExecutor>(_executorCount, stoppingToken);
            ...
            using (JobsRepo jobRepo = new JobsRepo())
            {
                while (stoppingToken.IsCancellationRequested == false)
                {
                    try
                    {
                        ...
                        //Parallel.ForEach(readyJobs, (jobInfo) =>
                        //{
                        //    _squad.AssignJob(jobInfo);
                        //});
